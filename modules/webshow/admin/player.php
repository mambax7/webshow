...
function menuPlayer()
{
    ...
    $playerid         = isset($_GET['playerid']) ? (int)$_GET['playerid'] : 1;
...
function newPlayer()
{
    ...
    $styleoption = isset($_GET['styleoption']) ? (int)$_GET['styleoption'] : $styleoption;

    //** THEME FROM PLAYER
    //** Uses the color style from this player for the pages color style.
    if ($styleoption == '4') {
        include_once dirname(__DIR__) . '/include/playerstyle.inc.php';
    ...
    echo '<div>';
    include dirname(__DIR__) . '/include/playerform.inc.php';
    echo '</div>';
    ...
    if ($styleoption == 2) {
        ...
        //** THEME COLOR EDITOR
        include __DIR__ . '/themestyle.php';
        ...
        if (!empty($lid)) {
            include dirname(__DIR__) . '/include/movie.inc.php';
...
function clonePlayer()
{
    ...
    //adminmenu(1);
    $lid         = isset($_GET['lid']) ? (int)$_GET['lid'] : '';
    $playerid    = isset($_GET['playerid']) ? (int)$_GET['playerid'] : redirect_header('javascript:history.go(-1)', 2, _WS_NOTEXIST);
    $playertitle = '';
...
function newPlayerS()
{
    ...
    $pid         = isset($_POST['pid']) ? (int)$_POST['pid'] : '';  //Unused atm
    ...
    $styleoption   = $myts->addSlashes((int)$_POST['styleoption']);
    ...
    $displayheight = $myts->addSlashes((int)$_POST['displayheight']);
    $showdigits    = $myts->addSlashes((int)$_POST['showdigits']);
    $showfsbutton  = $myts->addSlashes((int)$_POST['showfsbutton']);
    $scroll        = $myts->addSlashes((int)$_POST['scroll']);
    $width         = $myts->addSlashes((int)$_POST['width']);
    $height        = $myts->addSlashes((int)$_POST['height']);
    $displaywidth  = $myts->addSlashes((int)$_POST['displaywidth']);
    $largecontrol  = $myts->addSlashes((int)$_POST['largecontrol']);
    $searchbar     = $myts->addSlashes((int)$_POST['searchbar']);
    $searchlink    = $myts->addSlashes($_POST['searchlink']);
    $newid         = $xoopsDB->genId($xoopsDB->prefix('webshow_player') . '_playerid_seq');

    $sql =
        sprintf("INSERT INTO %s (playerid, pid, playertitle, styleoption, bgcolor, backcolor, frontcolor, lightcolor, displaywidth, displayheight, showdigits, showfsbutton, scroll, width, height, largecontrol, searchbar, searchlink) VALUES (%u, %u, '%s', %u, '%s', '%s', '%s', '%s', %u, %u, %u, %u, %u, %u, %u, %u, '%s', %u)",
                $xoopsDB->prefix('webshow_player'), $newid, $pid, $playertitle, $styleoption, $bgcolor, $backcolor, $frontcolor, $lightcolor, $displaywidth, $displayheight, $showdigits, $showfsbutton,
                $scroll, $width, $height, $largecontrol, $searchbar, $searchlink);
...
function modPlayerS()
{
    ...
    $pid         = ''; //$pid = (int)($_POST['pid']);  //unused atm
    $playertitle = isset($_POST['playertitle']) ? $_POST['playertitle'] : '';
    if (empty($playertitle)) {
        redirect_header('player.php', 2, _WSA_ERRORTITLE);
    }
    ...
    $xoopsDB->query('update '
                    ...
                    . " set pid='$pid', playertitle='$playertitle', styleoption='$styleoption', bgcolor='$bgcolor', backcolor='$backcolor', frontcolor='$frontcolor', lightcolor='$lightcolor', displayheight='$displayheight', showdigits='$showdigits', showfsbutton = '$showfsbutton', scroll='$scroll', width='$width', height='$height', displaywidth='$displaywidth', largecontrol='$largecontrol', searchbar='$searchbar', searchlink='$searchlink' WHERE playerid=$playerid")
    or $eh->show('0013');
...
function delPlayer()
{
    ...
    $playerid = isset($_POST['playerid']) ? (int)$_POST['playerid'] : (int)$_GET['playerid'];
