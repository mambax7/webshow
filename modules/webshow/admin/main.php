...
function webShow()
{
    ...
    //** Links Select
    echo "<tr class='odd'><td><form method=get action=\"main.php\">";
    $linktree->makeMySelBox('title', 'title', 0, 1, '', 'window.location="main.php?op=modLink&amp;lid="+this.value');
    echo "<input type=\"button\" value=\""
         . _WSA_ADDMEDIA
         . "\" onClick=\"location='main.php?op=newLink'\"> <input type=\"button\" value=\""
         . _WSA_LOGFILE
         . "\" onClick=\"location='main.php?op=showLog'\"></form></td></tr>";
    //**Player Select
    ...
    echo "<tr class='odd'><td><a href=main.php?op=listNewLinks>"
         . _WSA_LINKSWAITING
         . " ($totalnewlinks)</a> | <a href=main.php?op=listBrokenLinks>"
         . _WSA_BROKENREPORTS
         . " ($totalbrokenlinks)</a> | <a href=main.php?op=listModReq>"
         . _WSA_MODREQUESTS
         . " ($totalmodrequests)</a><br></td></tr>";
...
function menuLink($case)
{
    ...
    $show = isset($_GET['show']) ? (int)$_GET['show'] : ($xoopsModuleConfig['perpage'] * 2);
   ...
   <tr><th colspan=\"14\"><h3>" . _WSA_LINKMGMT . "</h3>
    <form style=\"float: left; width: 60%;\" name=\"sortform\" id=\"sortform\">
      <select name=\"orderby\" onChange=\"location = this.options[this.selectedIndex].value;\">
        ...
        <option value=\"main.php?op=" . $case . '&amp;orderby=titleA&amp;lid=' . $lid . ";\">" . _WS_TITLEATOZ . "</option>
        <option value=\"main.php?op=" . $case . '&amp;orderby=titleD&amp;lid=' . $lid . "\">" . _WS_TITLEZTOA . "</option>
        <option value=\"main.php?op=" . $case . '&amp;orderby=dateD&amp;lid=' . $lid . ";\">" . _WS_DATENEW . "</option>
        <option value=\"main.php?op=" . $case . '&amp;orderby=dateA&amp;lid=' . $lid . ";\">" . _WS_DATEOLD . "</option>
        <option value=\"main.php?op=" . $case . '&amp;orderby=ratingD&amp;lid=' . $lid . ";\">" . _WS_RATINGHTOL . "</option>
        <option value=\"main.php?op=" . $case . '&amp;orderby=ratingA&amp;lid=' . $lid . ";\">" . _WS_RATINGLTOH . "</option>
        <option value=\"main.php?op=" . $case . '&amp;orderby=votesD&amp;lid=' . $lid . ";\">" . _WS_VOTESMTOL . "</option>
        <option value=\"main.php?op=" . $case . '&amp;orderby=votesA&amp;lid=' . $lid . ";\">" . _WS_VOTESLTOM . "</option>
        <option value=\"main.php?op=" . $case . '&amp;orderby=hitsD&amp;lid=' . $lid . ";\">" . _WS_PAGEHITSMTOL . "</option>
        <option value=\"main.php?op=" . $case . '&amp;orderby=hitsA&amp;lid=' . $lid . ";\">" . _WS_PAGEHITSLTOM . "</option>
        <option value=\"main.php?op=" . $case . '&amp;orderby=viewsD&amp;lid=' . $lid . ";\">" . _WS_VIEWSMTOL . "</option>
        <option value=\"main.php?op=" . $case . '&amp;orderby=viewsA&amp;lid=' . $lid . ";\">" . _WS_VIEWSLTOM . "</option>
        <option value=\"main.php?op=" . $case . '&amp;orderby=lidD&amp;lid=' . $lid . ";\">" . _WS_LIDHTOL . "</option>
        <option value=\"main.php?op=" . $case . '&amp;orderby=lidA&amp;lid=' . $lid . ";\">" . _WS_LIDLTOH . "</option>
      </select>
      <input type='button' value=\"" . _WSA_ADDMEDIA . "\" onClick=\"location='main.php?op=newLink'\">
      <input type=\"button\" value=\"" . _WSA_LOGFILE . "\" onClick=\"location='main.php?op=showLog'\">
     </form>
      <input type=\"text\" value=\"" . $lid . "\" size=\"8\" maxlength=\"8\" name=\"getid\">
      <input type=\"button\" value=\"" . _WSA_GETID . "\" onClick=\"location='main.php?op=modLink&amp;lid='+getid.value\">
   </th></tr>
   <tr><td>
    ...
    $sql    =
        'SELECT lid, cid, title, listtype, srctype, player, status, date, published, expired, hits, views, rating, votes FROM ' . $xoopsDB->prefix('webshow_links') . " WHERE lid>0 order by $orderby";
    ...
    while (list($lid, $cid, $title, $listtype, $srctype, $playerid, $status, $date, $published, $expired, $hits, $views, $rating, $votes) = $xoopsDB->fetchRow($result)) {
        ...
        if ($status == 0) {
            ...
            $statuslink   = 'main.php?op=listNewLinks';
        ...
        if ($status == -1) {
            ...
            if ($date == 0 & $published > time()) {
                ...
                $statuslink   = 'main.php?op=modLink&amp;lid=' . $lid;
        ...
        if ($modrequest) {
            ...
            $statuslink   = 'main.php?op=listModReq';
            $statusicon   = 'offline.gif';
        }
        //if ($statusreport) {
        ...
        $playerpath = "<a href=\""
                      ...
                      . '/admin/player.php?op=modPlayer&amp;playerid='
                      . $playerid
                      . "\" title=\""
                      . _WSA_MODPLAYER
                      . "\">"
        ...
        print "   <td width=\"10%\">
                     <a href=\"main.php?op=modLink&amp;lid=$lid\" onClick=\"location='main.php?op=modLink&amp;lid=$lid'\" title=\""
              . _WS_MODIFY
              . "\"><img src=\"../assets/images/modify.gif\"></a>
                        <a href=\"main.php?op=delLink&amp;lid=$lid\" onClick=\"location='main.php?op=delLink&amp;lid=$lid'\" title=\""
              . _DELETE
              . "\"><img src=\"../assets/images/delete.gif\"></a>
                     <a href=\""
              . $statuslink
              . "\" title=\""
              . $statusreport
              ...
              . "\"></a>
                  ...
                  <td><a href=\"main.php?op=modLink&amp;lid=$lid\" onClick=\"location='main.php?op=modLink&amp;lid=$lid'\" title=\""
              . _WS_MODIFY
              . "\">"
              . $lid
              . "</a></td>
              ...
              . $rating
              . "</td>
                  <td><a href='main.php?op=voteStats&amp;lid="
              . $lid
              . "' title=\""
              . $title
              . ' '
              . _WSA_VOTESTATS
              . "\" onClick= \"location='main.php?op=voteStats&amp;lid="
              . $lid
              . "'\">"
              . $votes
              . '</a></td>
    ...
    /*
       ...
       if ($linkpages!=1 && $linkpages!=0) {
          $prev = $min - $show;
          if ($prev>=0) {
              $page_nav .= "<a href='main.php?op=".$case."&amp;lid=$lid&amp;min=$prev&amp;orderby=$orderby&amp;show=$show'><b><u>&laquo;</u></b></a>&nbsp;";
          }
          $counter = 1;
          ...
          while ($counter<=$linkpages) {
             ...
             if ($counter == $currentpage) {
                 ...
                 $page_nav .= "<a href='main.php?op=".$case."&amp;lid=$lid&amp;min=$mintemp&amp;orderby=$orderby&amp;show=$show'>$counter</a>&nbsp;";
             }
             ++$counter;
          }
          if ($numrows>$max) {
             $page_nav .= "<a href='main.php?op=".$case."&amp;lid=$lid&amp;min=$max&amp;orderby=$orderby&amp;show=$show'>";
...
function newLink()
{
    ...
    $ws_entryperm  = array();
    for ($i = 0; $i < $count; ++$i) {
        $ws_entryperm[] = $xoopsgroups[$i]->getVar('groupid');
    }
    $ws_entrydownperm = array();
    for ($i = 0; $i < $count; ++$i) {
        $ws_entrydownperm[] = $xoopsgroups[$i]->getVar('groupid');
    }
    ...
    if ($numrows == 0) {
        ...
        $wsformaction   = XOOPS_URL . '/modules/' . $xoopsModule->getVar('dirname') . '/admin/main.php?op=newLink';
        $wsformmethod   = 'post';
        ...
        include dirname(__DIR__) . '/include/submitform.inc.php';
        $sform->display();
        echo '</td></tr></table>';
    }
    xoops_cp_footer();
...
function addLink()
{
    include dirname(__DIR__) . '/include/submit.functions.php';
    saveEntry('addLink');
}
...
function delLink()
{
    ...
    $lid = $_GET['lid'] ? (int)$_GET['lid'] : (int)$_POST['lid'];
    ...
    if ($ok == 1) {
        ...
        deleteEntrylogo($lid);
        // Delete Playlist
        include dirname(__DIR__) . '/include/playlist.php';
        deletePlaylist($lid);
        ...
        redirect_header('main.php?op=newLink', 1, _WSA_LINKDELETED);
    } else {
        xoops_cp_header();
        xoops_confirm(array('op' => 'delLink', 'lid' => $lid, 'ok' => 1), 'main.php', _WSA_LINKWARNING);
        xoops_cp_footer();
    }
}
...
function modLink()
{
    ...
    if ($lid == '') {
        redirect_header(XOOPS_URL . '/modules/' . $xoopsModule->getVar('dirname') . '/admin/main.php?op=newLink', 0, '');
    }
    $result =
        $xoopsDB->query('select cid, title, url, srctype, listtype, listurl, listcache, status, entryinfo, submitter, logourl, player, credit1, credit2, credit3, cachetime, allowcomments, comments, date, expired, published, entryperm, entrydownperm from '
                        . $xoopsDB->prefix('webshow_links')
                        . " where lid=$lid") or $eh->show('0013');
    list($cid, $title, $url, $srctype, $listtype, $listurl, $listcache, $status, $entryinfo, $submitter, $logourl, $playerid, $credit1, $credit2, $credit3, $cachetime, $allowcomments, $comments,
        $date, $expired, $published, $ws_entryperm, $ws_entrydownperm) = $xoopsDB->fetchRow($result);
    ...
    $wsformaction   = XOOPS_URL . '/modules/' . $xoopsModule->getVar('dirname') . '/admin/main.php?op=modLink';
    $wsformmethod   = 'post';
    $wsformaddtoken = 'false';
    $op             = 'modLinkS';
    $btnlabel       = _WS_MODIFY;
...
function listNewLinks()
{
    ...
    $result  =
        $xoopsDB->query('select lid, cid, title, url, srctype, listtype, listurl, listcache, entryinfo, logourl, submitter, player, credit1, credit2, credit3, cachetime, allowcomments, comments, date, published, expired, entryperm, entrydownperm from '
                        . $xoopsDB->prefix('webshow_links')
    ...
    if ($numrows == 0) {
        ...
        while (list($lid, $cid, $title, $url, $srctype, $listtype, $listurl, $listcache, $entryinfo, $logourl, $submitter, $playerid, $credit1, $credit2, $credit3, $cachetime, $allowcomments,
            $comments, $date, $published, $expired, $ws_entryperm, $ws_entrydownperm) = $xoopsDB->fetchRow($result)) {
            $result2 = $xoopsDB->query('select description, bodytext from ' . $xoopsDB->prefix('webshow_text') . " where lid=$lid");
            list($description, $bodytext) = $xoopsDB->fetchRow($result2);
            $title     = $myts->htmlSpecialChars($title);
            $url       = $myts->htmlSpecialChars($url);
            ...
            $wsformaction   = XOOPS_URL . '/modules/' . $xoopsModule->getVar('dirname') . '/admin/main.php?op=listNewLinks';
            $wsformmethod   = 'post';
            $wsformaddtoken = 'false';
            $op             = 'approve';
...
function listBrokenLinks()
{
    ...
    if ($totalbrokenlinks == 0) {
        ...
        while (list($reportid, $lid, $sender, $ip, $rpttype, $rptname, $rptcmt) = $xoopsDB->fetchRow($result)) {
            ...
            echo myTextForm("main.php?op=ignoreBrokenLinks&reportid=$reportid", 'X');
            echo "</td><td bgcolor='$colorswitch' align='center'>\n";
...
function delBrokenLinks()
{
    ...
    include dirname(__DIR__) . '/include/playlist.php';
    $lid = isset($_POST['lid']) ? (int)$_POST['lid'] : die('Restricted access');
    $ok  = isset($_POST['ok']) ? (int)$_POST['ok'] : 0;
    if ($ok == 1) {
        deletePlaylist($lid);
        ...
        // delete notifications
        xoops_notification_deletebyitem($xoopsModule->getVar('mid'), 'link', $lid);
        redirect_header('main.php?op=listBrokenLinks', 1, _WSA_LINKDELETED);
    } else {
        xoops_cp_header();
...
{
    global $xoopsDB, $eh;
    $sql = sprintf('DELETE FROM %s WHERE reportid = %u', $xoopsDB->prefix('webshow_broken'), (int)$_GET['reportid']);
    $xoopsDB->query($sql) or $eh->show('0013');
    redirect_header('main.php?op=listBrokenLinks', 1, _WSA_BROKENDELETED);
}
...
function listModReq()
{
    ...
    $modresult        = $xoopsDB->query('SELECT requestid, lid, cid, title, url, listurl, logourl, credit1, credit2, credit3, description, bodytext, modifysubmitter, ws_tag FROM '
                                        . $xoopsDB->prefix('webshow_mod')
                                        . ' order by requestid');
    ...
    if ($totalmodrequests > 0) {
        ...
        while (list($requestid, $lid, $cid, $title, $url, $listurl, $logourl, $credit1, $credit2, $credit3, $description, $bodytext, $submitterid, $ws_tag) = $xoopsDB->fetchRow($modresult)) {
            ...
            $origresult1            =
                $xoopsDB->query('SELECT cid, title, url, listurl, logourl, submitter, player, credit1, credit2, credit3 FROM ' . $xoopsDB->prefix('webshow_links') . " WHERE lid=$lid");
            ...
            echo "<td align=right>\n";
            echo "<table><tr><td>\n";
            echo myTextForm("main.php?op=changeModReq&requestid=$requestid", _WSA_APPROVE);
            echo "</td><td>\n";
            echo myTextForm("main.php?op=modLink&lid=$lookup_lid[$requestid]", _EDIT);
            echo "</td><td>\n";
            echo myTextForm("main.php?op=ignoreModReq&requestid=$requestid", _WSA_IGNORE);
            echo "</td></tr></table>\n";
            echo "</td></tr>\n";
            echo '</table><br><br>';
        }
...
function changeModReq()
{
    ...
    $query     = 'select lid, cid, title, url, srctype, listurl, logourl, player, credit1, credit2, credit3, description, bodytext from '
                 . $xoopsDB->prefix('webshow_mod')
                 . ' where requestid='
                 . $requestid
                 . '';
    ...
    while (list($lid, $cid, $title, $url, $srctype, $listurl, $logourl, $player, $credit1, $credit2, $credit3, $description, $bodytext) = $xoopsDB->fetchRow($result)) {
        ...
        $sql         =
            sprintf("UPDATE %s SET cid = %u, title = '%s', url = '%s', srctype='%s', listurl = '%s', logourl = '%s', status = %u, date = %u, player=%u, credit1 = '%s', credit2 = '%s', credit3 = '%s' WHERE lid = %u",
                    $xoopsDB->prefix('webshow_links'), $cid, $title, $url, $srctype, $listurl, $logourl, 2, time(), $player, $credit1, $credit2, $credit3, $lid);
        $xoopsDB->query($sql) or $eh->show('0013');
    ...
    redirect_header('main.php?op= listModReq', 1, _WSA_DBUPDATED);
}

function ignoreModReq()
{
    global $xoopsDB, $eh;
    $sql = sprintf('DELETE FROM %s WHERE requestid = %u', $xoopsDB->prefix('webshow_mod'), (int)$_GET['requestid']);
    $xoopsDB->query($sql) or $eh->show('0013');
    redirect_header('main.php?op=listModReq', 1, _WSA_MODREQDELETED);
}
...
function webShowHelp()
{
    ...
    xoops_cp_header();
    //adminmenu(1);
    include __DIR__ . '/docs/help.html';
    xoops_cp_footer();
}
...
function ws_admin_getPathStatus($path)
{
    ...
    if (@is_writable($path)) {
        ...
        $path_status = "<font color=\"red\">"
                       . _WSA_NOTAVAILABLE
                       . '</font> <a href='
                       . XOOPS_URL
                       . '/modules/'
                       . $xoopsModule->getVar('dirname')
                       . "/admin/main.php?op=createdir&amp;path=$path>"
                       . _WSA_CREATETHEDIR
                       . '</a>';
    } else {
        $path_status = "<font color=\"red\">"
                       ...
                       . '/modules/'
                       . $xoopsModule->getVar('dirname')
                       . "/admin/main.php?op=setperm&amp;path=$path>"
                       . _WSA_SETMPERM
                       . '</a>';
    }
...
function voteStats()
{
   ...
   <tr><th colspan=\"7\"><h3>" . _WSA_VOTESTATS . "</h3>
    ...
    <div style=\"float: right; width: 35%; text-align: right;\">
      <input type=\"button\" value=\"" . _WSA_LINKMGMT . "\" onClick=\"location='main.php?op=newLink'\">
      <input type=\"button\" value=\"" . _WSA_LOGFILE . "\" onClick=\"location='main.php?op=showLog'\">
    </div></th></tr>";
    ...
    if ($lid) {
        ...
        $result5 = $xoopsDB->query('SELECT ratingid, ratinguser, rating, ratinghostname, ratingtimestamp FROM '
                                   . $xoopsDB->prefix('webshow_votedata')
        ...
        while (list($ratingid, $ratinguser, $rating, $ratinghostname, $ratingtimestamp) = $xoopsDB->fetchRow($result5)) {
            ...
            print "<tr><td bgcolor=\""
                  ...
                  . $uservotes
                  . "</td><td bgcolor=\"$colorswitch\">"
                  . formatTimestamp($ratingtimestamp)
                  . "</td><td bgcolor=\"$colorswitch\" align=\"center\"><a href=\"main.php?op=delVote&lid=$lid&rid=$ratingid\" title=\""
                  . _DELETE
                  . "\"><img src=\"../assets/images/delete.gif\"></a></td></tr>\n";
            ++$x;
            if ($colorswitch == 'dddddd') {
                $colorswitch = 'ffffff';
            } else {
        ...
        $result5 = $xoopsDB->query('SELECT ratingid, rating, ratinghostname, ratingtimestamp FROM '
                                   . $xoopsDB->prefix('webshow_votedata')
                                   . " WHERE lid = $lid AND ratinguser = 0 ORDER BY ratingtimestamp DESC");
        $votes   = $xoopsDB->getRowsNum($result5);
        ...
        print '<tr><td colspan=2><strong>'
              . _WSA_IP
              . '  </strong></td><td colspan=3><strong>'
              . _WSA_RATING
              . '  </strong></td><td><strong>'
              . _DATE
              . "  </strong></strong></td><td align=\"center\"><strong>"
              . _DELETE
              . '</strong></td><br></tr>';
        if ($votes == 0) {
        ...
        while (list($ratingid, $rating, $ratinghostname, $ratingtimestamp) = $xoopsDB->fetchRow($result5)) {
            ...
            print "<td colspan=\"2\" bgcolor=\"$colorswitch\">$ratinghostname</td><td colspan=\"3\" bgcolor=\"$colorswitch\">$rating</td><td bgcolor=\"$colorswitch\">$formatted_date</td><td bgcolor=\"$colorswitch\" align=\"center\"><a href=\"main.php?op=delVote&lid=$lid&rid=$ratingid\" title=\""
                  . _DELETE
                  . "\"><img src=\"../assets/images/delete.gif\"></a></td></tr>";
            ++$x;
            if ($colorswitch == 'dddddd') {
...
function showLog()
{
   ...
   <th colspan=\"8\"><h3>"
      ...
      <input type=\"button\" value=\""
          . _WSA_LINKMGMT
          . "\" onClick=\"location='main.php?op=newLink'\">
      <input type=\"button\" value=\""
          . _WSA_EMPTYLOG
          . "\" onClick=\"location='main.php?op=emptyLog'\">
   </th>
   <tr><td>
     <div style=\"height: 300px; width: 700px; overflow: auto;\">
       <table border=\"1\" style=\"font-size: 85%;\">
        <tr class=\"head\" style=\"font-weight: 600;\" colspan=\"8\"><td>"
          . _WSA_LINENUMBER
          . '</td><td>'
          . _DATE
          . '</td><td>'
...
function emptyLog()
{
    ...
    redirect_header('main.php?op=showLog', 3, _WSA_EMPTYLOG);
}
