...
function fvConfig()
{
    ...
    $btnlabel       = _WS_MODIFY;
    echo '<tr><td>';
    include dirname(__DIR__) . '/include/flvarform.inc.php';
    echo '</td></tr></table>';
...
function fvConfigS()
{
    ...
    $lid      = isset($_POST['lid']) ? (int)$_POST['lid'] : redirect_header('javascript:history.go(-1)', 2, _WSA_NOTALLOWED);
    $listtype = '';
    ...
    if (!empty($_FILES['userfile2']['name'])) {
        ...
        $uploader2 =
            new XoopsMediaUploader(XOOPS_ROOT_PATH . '/modules/' . $xoopsModule->getVar('dirname') . '/assets/images/player', $allowed_mimetypes, $playerlogosize, $playerlogowidth, $playerlogoheight);
...
{
    global $xoopsDB, $eh;
    $lid = isset($_GET['lid']) ? (int)$_GET['lid'] : (int)$_POST['lid'];
    if ($lid > 0) {
        // Get GLOBAL Defaults from Row 0
        $result =
            $xoopsDB->query('select start, shuffle, replay, link, linktarget, showicons, stretch, showeq, rotatetime, shownav, transition, thumbslist, captions, enablejs, playerlogo, audio from '
                            . $xoopsDB->prefix('webshow_flashvar')
                            . ' where lid=0') or $eh->show('0013');
        list($start, $shuffle, $replay, $link, $linktarget, $showicons, $stretch, $showeq, $rotatetime, $shownav, $transition, $thumbslist, $captions, $enablejs, $playerlogo, $audio) =
            $xoopsDB->fetchRow($result);
        $xoopsDB->queryF('update '
                         . $xoopsDB->prefix('webshow_flashvar')
                         . " set start='$start', shuffle='$shuffle', replay='$replay', link='$link', linktarget='$linktarget', showicons='$showicons', stretch='$stretch', showeq='$showeq', rotatetime='$rotatetime', shownav='$shownav', transition='$transition', thumbslist='$thumbslist', captions='$captions', enablejs='$enablejs', playerlogo='$playerlogo', audio='$audio' where lid=$lid")
        or $eh->show('0013');
    } else {
        //** FLASH VARIABLES
        ...
        $xoopsDB->queryF('update '
                         . $xoopsDB->prefix('webshow_flashvar')
                         . " set start='$start', shuffle='$shuffle', replay='$replay', link='$link', linktarget='$linktarget', showicons='$showicons', stretch='$stretch', showeq='$showeq', rotatetime='$rotatetime', shownav='$shownav', transition='$transition', thumbslist='$thumbslist', captions='$captions', enablejs='$enablejs', playerlogo='$playerlogo', audio='$audio' where lid=0")
        or $eh->show('0013');
    }
    redirect_header("flashconfig.php?op=fvConfig&amp;lid=$lid", 3, _WSA_DBUPDATED);
}
...
function checkFVdefrow()
{
    ...
    if (!$xoopsDB->getRowsNum($result) > 0) {
        ...
        $xoopsDB->queryF('INSERT INTO '
                         . $xoopsDB->prefix('webshow_flashvar')
                         . " (lid, start, shuffle, replay, link, linktarget, showicons, stretch, showeq, rotatetime, shownav, transition, thumbslist, captions, enablejs, playerlogo, audio) VALUES ( '0', '$start', '$shuffle', '$replay', '$link', '$linktarget', '$showicons', '$stretch', '$showeq', '$rotatetime', '$shownav', '$transition', '$thumbslist', '$captions', '$enablejs', '$playerlogo', '$audio')")
        or $eh->show('0013');
    }
