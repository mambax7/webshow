...
function menuCat($case)
{
   ...
   <tr><td>
    ...
    while (list($cid, $pid, $cattitle, $imgurl) = $xoopsDB->fetchRow($catresult)) {
        ...
        $catimgurl = !empty($imgurl) ? "<img src=\"../assets/images/category/" . $imgurl . "\">" : "<img src=\"../assets/imagesblank.gif\">";
        if (!$pid) {
            $parentpath = '---';
        } else {
        ...
        echo "   <td width=\"12%\">
                     <a href=\"category.php?op=modCat&amp;cid=$cid\" onClick=\"location='category.php?op=modCat&amp;cid=$cid'\" title=\"" . _WS_MODIFY . "\"><img src=\"../assets/images/modify.gif\"></a>
                        <a href=\"category.php?op=delCat&amp;cid=$cid\" onClick=\"location='category.php?op=delCat&amp;cid=$cid'\" title=\"" . _DELETE . "\"><img src=\"../assets/images/delete.gif\"></a>
                     <a href=\"../playcat.php?cid=$cid\" onClick=\"location='../playcat.php?cid=$cid'\" title=\"" . _WSA_VIEWFORM_DESC . "\"><img src=\"../assets/images/link.gif\"></a>
                     </td>
                  <td>
...
function newCat()
{
    ...
    $path_catimg    = 'modules/' . $xoopsModule->getVar('dirname') . '/assets/images/category';
...
function addCat()
{
    ...
    if (!empty($_FILES['userfile']['name'])) {
        ...
        $uploader =
            new XoopsMediaUploader(XOOPS_ROOT_PATH . '/modules/' . $xoopsModule->getVar('dirname') . '/assets/images/category/', $allowed_mimetypes, $maxfilesize, $maxfilewidth, $maxfileheight);
    ...
    $sql   =
        sprintf("INSERT INTO %s (cid, pid, cattitle, catdesc, imgurl, catbody) VALUES (%u, %u, '%s', '%s', '%s', '%s')", $xoopsDB->prefix('webshow_cat'), $newid, $pid, $cattitle, $catdesc, $imgurl,
                $catbody);
