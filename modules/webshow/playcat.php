...
$xoopsOption['template_main'] = 'webshow_playcat.tpl';
    ...
    foreach ($arr as $ele) {
        if ($gperm_handler->checkRight('webshow_view', $ele['cid'], $groups, $xoopsModule->getVar('mid'))) {
            ...
            $xoopsTpl->append('subcategories', array(
                'cattitle'        => $myts->htmlSpecialChars($ele['cattitle']),
                'id'              => $ele['cid'],
...
if ($numrows > 0) {
    ...
    while (list($lid, $cid, $title, $url, $srctype, $listtype, $listurl, $entryinfo, $listcache, $cachetime, $logourl, $credit1, $credit2, $credit3, $submitter, $status, $date, $published, $hits,
        $rating, $votes, $allowcomments, $comments, $player, $ws_entryperm, $ws_entrydownperm, $views, $description) = $xoopsDB->fetchRow($result)) {
        ...
        if (count(array_intersect($group, $ws_entryperm)) > 0) {
            ...
            if (!$showplayer) {
                ...
                $title = "<a href=\""
                         ...
                         . '/modules/'
                         . $xoopsModule->getVar('dirname')
                         . "/playcat.php?lid=$lid&amp;cid=$cid&amp;uid=$submitter&amp;min=$rowid&amp;orderby=$orderby&amp;show=$show\" target=\"_self\" title=\""
                         . _WS_VIEWMEDIA
                         . ' - '
                         . $ltitle
                         . "\" rel=\"nofollow\">"
            ...
            if ($showplayer & !$playeroff & $lid == $movielid) {
                ...
                $playerresult =
                    $xoopsDB->query('select bgcolor, backcolor, frontcolor, lightcolor, width, height, styleoption from ' . $xoopsDB->prefix('webshow_player') . " where playerid=$playerid");
