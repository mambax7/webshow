...
function wsrateform()
{
    ...
    $lid       = isset($_GET['lid']) ? (int)$_GET['lid'] : redirect_header('index.php', 2, _WS_NOTALLOWED);
    $rating    = '';
...
function wsrate()
{
    ...
    if ($ratinguser != 0) {
        ...
        $result       =
            $xoopsDB->queryF('select count(*) FROM ' . $xoopsDB->prefix('webshow_votedata') . " WHERE lid=$lid AND ratinguser=0 AND ratinghostname = '$ip' AND ratingtimestamp > $yesterday");
    ...
    $datetime = time();
    $sql      = sprintf("INSERT INTO %s (ratingid, lid, ratinguser, rating, ratinghostname, ratingtimestamp) VALUES (%u, %u, %u, %u, '%s', %u)", $xoopsDB->prefix('webshow_votedata'), $newid, $lid,
...
function wsratenew()
{
    ...
    $newrate = '<div style="float: left;"><strong>'
               . _WS_RATING
               . ': </strong>'
               . $rating
               . ' ('
               . $votestring
               . ')</div><div class="rating_bar" style="float: left;"><div style="width:'
               . ($rating * 10)
               . '%"></div></div>';
    print $newrate;
}
