...
$xoopsOption['template_main'] = 'webshow_singlelink.tpl';
...
//** Get the entry data.
$lid    = isset($_GET['lid']) ? (int)$_GET['lid'] : redirect_header('index.php', 2, _WS_NOTEXIST);
$result =
    $xoopsDB->query('select l.lid, l.cid, l.title, l.url, l.srctype, l.listtype, l.listurl, l.entryinfo, l.listcache, l.cachetime, l.logourl, l.credit1, l.credit2, l.credit3, l.submitter, l.status, l.date, l.published, l.expired, l.hits, l.rating, l.votes, l.allowcomments, l.comments, l.player, l.entryperm, l.entrydownperm, l.views, t.description, t.bodytext from '
                    . $xoopsDB->prefix('webshow_links')
                    . ' l, '
                    . $xoopsDB->prefix('webshow_text')
                    . " t where l.lid=$lid and l.lid=t.lid");
list($lid, $cid, $title, $url, $srctype, $listtype, $listurl, $entryinfo, $listcache, $cachetime, $logourl, $credit1, $credit2, $credit3, $submitter, $status, $date, $published, $expired, $hits,
    $rating, $votes, $allowcomments, $comments, $playerid, $ws_entryperm, $ws_entrydownperm, $views, $description, $bodytext) = $xoopsDB->fetchRow($result);
...
$xoopsTpl->assign('link', array(
    ...
    'mail_body'     => rawurlencode(sprintf(_WS_INTLINKFOUND, $xoopsConfig['sitename']) . ':  ' . XOOPS_URL . '/modules/' . $xoopsModule->getVar('dirname') . '/singlelink.php?lid=' . $lid),
    'bodytext'      => $bodytext,
    'hostname'      => $hostname,
    'hostlink'      => $hostlink
));
...
if ($listtype == 'feed' || $listtype == 'dir') {
    include __DIR__ . '/include/feeddata.inc.php';
}
