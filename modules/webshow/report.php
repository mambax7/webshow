...
function wsreportform()
{
    ...
    $lid       = isset($_GET['lid']) ? (int)$_GET['lid'] : redirect_header('javascript:history.go(-1)', 2, _WS_NOTALLOWED);
    $rpttype   = '';
    $rptcmt    = '';
    ...
    //** Description
    $sform->addElement(new XoopsFormTextArea('<b>' . _WS_REPORTCOMMENT . ": </b><br><span style=\"font-size: 85%; font-weight: 500;\">" . _WS_REPORTCOMMENT_DSC . '</span>', 'rptcmt', $rptcmt, 4, 30),
                       true);
...
function wsreport()
{
    ...
    $lid     = isset($_POST['lid']) ? (int)$_POST['lid'] : redirect_header('javascript:history.go(-1)', 2, _WS_NOTALLOWED);
    $rpttype = isset($_POST['rpttype']) ? (int)$_POST['rpttype'] : redirect_header('javascript:history.go(-1)', 2, _WS_NOTALLOWED);
    ...
    $sql   = sprintf("INSERT INTO %s (reportid, lid, sender, ip, rpttype, rptname, rptcmt) VALUES (%u, %u, %u, '%s', %u, '%s', '%s')", $xoopsDB->prefix('webshow_broken'), $newid, $lid, $sender, $ip,
                     $rpttype, $rptname, $rptcmt);
