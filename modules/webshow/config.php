...
$lid = isset($_GET['lid']) ? (int)$_GET['lid'] : exit();

$linkresult = $xoopsDB->query('select lid, cid, title, listurl, listcache, listtype, srctype, player, logourl, url, entryinfo, entryperm, entrydownperm from '
...
if ($styleoption == '2') {
    $theme       = isset($_GET['theme']) ? (int)$_GET['theme'] : $xoopsConfig['theme_set'];
...
if ($showdigits == 0) {
    $var .= '<showdigits>false</showdigits>';
} //default = true
if ($showfsbutton == 0) {
    $var .= '<usefullscreen>false</usefullscreen>';
} //default = true
if ($scroll == 1) {
    $var .= '<autoscroll>true</autoscroll>';
} //default=false
if ($largecontrol == 1) {
    $var .= '<largecontrol>true</largecontrol>';
} //default=false
...
//** FLASH VARIABLES
if ($start == 1) {
    $var .= '<autostart>true</autostart>';
}
if ($shuffle == 0) {
    $var .= '<shuffle>false</shuffle>';
} //default=true
if ($replay == 1) {
    $var .= '<repeat>true</repeat>';
} //default=false
...
if ($link != '0') {
    ...
    //** Screen Link Target
    if ($linktarget == '_self') {
        $var .= '<linktarget>' . $linktarget . '</linktarget>';
    }
    if ($link == 'site') {
        $var .= '<link>' . urlencode($url) . '</link>';
    }
    if ($link == 'page') {
...
if ($showicons == 0) {
    $var .= '<showicons>false</showicons>';
} //default = true
if ($stretch != 'false') {
    $var .= '<overstretch>' . $stretch . '</overstretch>';
}//default='false'
if ($showeq == 1) {
    $var .= '<showeq>true</showeq>';
}//default=false}
if ($thumbslist == 0) {
    $var .= '<thumbsinplaylist>false</thumbsinplaylist>';
}//default=true
if ($rotatetime != 0) {
    $var .= '<rotatetime>' . $rotatetime . '</rotatetime>';
}//default=0
if ($playerlogo != '') {
    $playerlogo = urlencode(XOOPS_URL . '/modules/' . $xoopsModule->getVar('dirname') . '/assets/images/player/' . $playerlogo);
...
if ($audio != '') {
    $var .= '<audio>' . urlencode($audio) . '</audio>';
} // default = ""
if ($searchbar == 1) {
