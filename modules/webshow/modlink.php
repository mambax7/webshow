...
function modLink()
{
    ...
    //*****  Modify Link Form
    $xoopsOption['template_main'] = 'webshow_modlink.tpl';
    include XOOPS_ROOT_PATH . '/header.php';

    $lid = (int)$_GET['lid'];
    $result =
        $xoopsDB->query('select cid, title, url, srctype, listtype, listurl, listcache, cachetime, status, entryinfo, submitter, logourl, player, credit1, credit2, credit3, cachetime, allowcomments, comments, date, expired, published, entryperm, entrydownperm from '
                        . $xoopsDB->prefix('webshow_links')
                        . " where lid=$lid") or $eh->show('0013');
    list($cid, $title, $url, $srctype, $listtype, $listurl, $listcache, $cachetime, $status, $entryinfo, $submitter, $logourl, $playerid, $credit1, $credit2, $credit3, $cachetime, $allowcomments,
        $comments, $date, $expired, $published, $ws_entryperm, $ws_entrydownperm) = $xoopsDB->fetchRow($result);
    ...
    //** FLASH VARIABLES
    $result3 = $xoopsDB->query('select start, shuffle, replay, link, linktarget, showicons, stretch, showeq, rotatetime, shownav, transition, thumbslist, captions, playerlogo, enablejs, audio from '
                               . $xoopsDB->prefix('webshow_flashvar')
                               . " where lid=$lid") or $eh->show('0013');
    list($start, $shuffle, $replay, $link, $linktarget, $showicons, $stretch, $showeq, $rotatetime, $shownav, $transition, $thumbslist, $captions, $playerlogo, $enablejs, $audio) =
        $xoopsDB->fetchRow($result3);
    ...
    $op             = 'modLinkS';
    $btnlabel       = _WS_MODIFY;
    include __DIR__ . '/include/submitform.inc.php';
