...
function saveEntry($op)
{
    ...
    $lid      = isset($_POST['lid']) ? (int)$_POST['lid'] : '';
    $cid      = (int)$_POST['cid'];
    ...
    if ($listtype != 'embed' && $listtype != 'dir') {
        $listurl = formatURL($listurl);
    }
    $listurl = $myts->addSlashes($listurl);

    //** CACHE PLAYLIST
    $cachetime   = isset($_POST['cachetime']) ? (int)$_POST['cachetime'] : '';
    $listcache   = isset($_POST['listcache']) ? $_POST['listcache'] : '';
    $refreshlist = isset($_POST['refreshlist']) ? (int)$_POST['refreshlist'] : '';
    $message     = '';
    ...
    //** Comments
    $allowcomments = isset($_POST['allowcomments']) ? (int)$_POST['allowcomments'] : '';
    ...
    //** Submitter
    $submitter = isset($_POST['submitter']) ? (int)$_POST['submitter'] : (!empty($xoopsUser) ? $xoopsUser->getVar('uid') : 0);

    //** Player
    $playerid = isset($_POST['playerid']) ? (int)$_POST['playerid'] : '1';  //If no player is selected, use Default player id #1
    ...
    if ($listtype != 'embed') {
        ...
        $shuffle    = isset($_POST['shuffle']) ? (int)$_POST['shuffle'] : 1;
        $replay     = isset($_POST['replay']) ? (int)$_POST['replay'] : 0;
        ...
        $showicons  = isset($_POST['showicons']) ? (int)$_POST['showicons'] : 1;
        $stretch    = isset($_POST['stretch']) ? $myts->addSlashes($_POST['stretch']) : 'false';
        $showeq     = isset($_POST['showeq']) ? (int)$_POST['showeq'] : 0;
        $rotatetime = isset($_POST['rotatetime']) ? (int)$_POST['rotatetime'] : 0;
        $shownav    = isset($_POST['shownav']) ? (int)$_POST['shownav'] : 0;
        $transition = isset($_POST['transition']) ? $myts->addSlashes($_POST['transition']) : 0;
        $thumbslist = isset($_POST['thumbslist']) ? (int)$_POST['thumbslist'] : 1;
        $captions   = isset($_POST['captions']) ? $myts->addSlashes(formatURL($_POST['captions'])) : '';
        $enablejs   = isset($_POST['enablejs']) ? (int)$_POST['enablejs'] : 0;
        ...
        if (!empty($_FILES['userfile2']['name'])) {
            ...
            $uploader2 =
                new XoopsMediaUploader(XOOPS_ROOT_PATH . '/modules/' . $xoopsModule->getVar('dirname') . '/images/player', $allowed_mimetypes, $playerlogosize, $playerlogowidth, $playerlogoheight);
    ...
    if ($op == 'addLink') {
        $autodate = isset($_POST['autodate']) ? (int)$_POST['autodate'] : 0;
        if ($autodate == 1) {
        ...
        //** CREATED DATE
        $autocreatedate = isset($_POST['autocreatedate']) ? (int)$_POST['autocreatedate'] : 0;
        if ($autocreatedate = 1) {
    ...
    //** EXPIRATION
    $autoexpire = isset($_POST['autoexpire']) ? (int)$_POST['autoexpire'] : '';
    if ($autoexpire != 1) {
        $expired = '';
    } else {
    ...
    if ($op == 'addLink') {
        ...
        $status = $_POST['status'] < 1 ? (int)$_POST['status'] : 2;
    }
    $statusswitch = isset($_POST['statusswitch']) ? (int)$_POST['statusswitch'] : '';
    if ($statusswitch == 0) {
    ...
    if ($op == 'addLink') {
        //** Insert data
        $sql =
            sprintf("INSERT INTO %s (lid, cid, title, url, srctype, listtype, listurl, listcache, entryinfo, submitter, status, date, published, expired, hits, rating, votes, allowcomments, comments, player, credit1, credit2, credit3, cachetime, entryperm, entrydownperm) VALUES (%u, %u, '%s', '%s', '%s', '%s', '%s', '%s', '%s', %u, '%s', %u, %u, %u, %u, %u, %u, %u, '%s', %u, '%s', '%s', '%s', %u, '%s', '%s')",
                    $xoopsDB->prefix('webshow_links'), $lid, $cid, $title, $url, $srctype, $listtype, $listurl, $listcache, $entryinfo, $submitter, $status, $date, $published, $expired, 0, 0, 0,
                    $allowcomments, $comments, $playerid, $credit1, $credit2, $credit3, $cachetime, $ws_entryperm, $ws_entrydownperm);
        ...
        if ($listtype != 'embed') {
            $sql =
                sprintf("INSERT INTO %s (lid, mediatype, start, shuffle, replay, link, linktarget, showicons, stretch, showeq, rotatetime, shownav, transition, thumbslist, captions, enablejs, playerlogo, audio) VALUES (%u, '%s', %u, %u, %u, %u, '%s', %u, '%s', %u, %u, %u, '%s', %u, '%s', %u, '%s', '%s')",
                        $xoopsDB->prefix('webshow_flashvar'), $lid, $mediatype, $start, $shuffle, $replay, $link, $linktarget, $showicons, $stretch, $showeq, $rotatetime, $shownav, $transition,
                        $thumbslist, $captions, $enablejs, $playerlogo, $audio);
            $xoopsDB->query($sql) or $eh->show('0013');
        }
    } else {
        if ($gperm_handler->checkRight('webshow_approve', $cid, $groups, $xoopsModule->getVar('mid'))) {
            ...
            $query = 'update '
                     ...
                     . " set cid='$cid', title='$title', url='$url', srctype='$srctype', listtype='$listtype', listurl='$listurl', listcache='$listcache', entryinfo='$entryinfo', status='$status', date='$date', published='$published', expired='$expired', allowcomments='$allowcomments', player='$playerid', credit1='$credit1', credit2='$credit2', credit3='$credit3', cachetime='$cachetime', entryperm='$ws_entryperm', entrydownperm='$ws_entrydownperm' where lid="
                     . $lid
                     . '';
            ...
            if ($listtype != 'embed') {
                $xoopsDB->query('update '
                                ...
                                . " set start='$start', shuffle='$shuffle', replay='$replay', link='$link', linktarget='$linktarget', showicons='$showicons', stretch='$stretch', showeq='$showeq', rotatetime='$rotatetime', shownav='$shownav', transition='$transition', thumbslist='$thumbslist', captions='$captions', enablejs='$enablejs', playerlogo='$playerlogo', audio='$audio' where lid="
                                . $lid
                                . '') or $eh->show('0013');
            }
        } else {
            ...
            $sql    =
                sprintf("INSERT INTO %s (requestid, lid, cid, title, url, srctype, listurl, description, bodytext, modifysubmitter, credit1, credit2, credit3, ws_tag) VALUES (%u, %u, %u, '%s', '%s', '%s', '%s', '%s', '%s', '%s', %u, '%s', '%s', '%s', '%s')",
                        $xoopsDB->prefix('webshow_mod'), $newid, $lid, $cid, $title, $url, $srctype, $listurl, $logourl, $description, $bodytext, $modifysubmitter, $credit1, $credit2, $credit3,
                        $ws_tag);
            $xoopsDB->query($sql) or $eh->show('0013');
            //** Save Flash Vars  // NEEDS WORK should be temp saved as array in the mod table
            $xoopsDB->query('update '
                            . $xoopsDB->prefix('webshow_flashvar')
                            . " set start='$start', shuffle='$shuffle', replay='$replay', link='$link', linktarget='$linktarget', showicons='$showicons', stretch='$stretch', showeq='$showeq', rotatetime='$rotatetime', shownav='$shownav', transition='$transition', thumbslist='$thumbslist', captions='$captions', playerlogo='$playerlogo', enablejs='$enablejs' where lid="
                            . $lid
                            . '') or $eh->show('0013');
        }
    }
