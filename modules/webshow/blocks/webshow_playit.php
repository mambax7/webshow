...
function b_webshow_playit_show($options)
{
    ...
    if ($options['2'] != 0) {
        //** Category Limit
        $result = $xoopsDB->query('SELECT lid, cid, title, listurl, srctype, listtype, listcache, entryinfo, logourl, date, hits, rating, votes, entryperm, views FROM '
                                  . $xoopsDB->prefix('webshow_links')
                                  . ' WHERE status > 0 and cid = '
                                  . $options[2]
                                  . ' '
                                  . $where
                                  . '');
    ...
    if ($myrow['lid'] == '') {
        if ($options['2'] != 0) {
            ...
            $result = $xoopsDB->query('SELECT lid, cid, title, listurl, srctype, listtype, listcache, entryinfo, logourl, date, hits, rating, votes, entryperm, views FROM '
                                      . $xoopsDB->prefix('webshow_links')
                                      . ' WHERE status>0 and cid>0 ORDER BY date DESC LIMIT 1');
        }
        $myrow = $xoopsDB->fetchArray($result);
    }
    ...
    if ($options[3] != '0') {
        ...
        $playerresult = $xoopsDB->query('select bgcolor, backcolor, frontcolor, lightcolor, width, height, displaywidth, displayheight, styleoption from '
        ...
        if ($listtype == 'embed') {
            ...
            $movie = '<script type="text/javascript" src="'
                     . $path_webshow
                     . '/assets/js/swfobject.js"></script><div id="play'
                     . $movielid
                     . '" style="width: '
                     . $width
                     . 'px ; float: left;"><a href="http://www.macromedia.com/go/getflashplayer">Get the Flash Player</a> to see this player.</div><script type="text/javascript">var s'
                     . $movielid
                     . ' = new SWFObject("'
                     . $path_webshow
                     . '/'
                     ...
                     . '","7");s'
                     . $movielid
                     . '.addParam("allowfullscreen","true");s'
                     . $movielid
                     . '.addVariable("title","'
                     . $title
                     . '");s'
