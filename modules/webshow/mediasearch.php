...
$xoopsOption['template_main'] = 'webshow_search.tpl';
...
//$redirect = !empty($_GET['redirect']) ? (int)($_GET['redirect']) : !empty($_POST['redirect']) ? (int)($_POST['redirect']) : '';
    ...
    if (in_array($mid, $available_modules)) {
        ...
        if (!is_array($results) || $numrows == 0) {
            ...
            */
            ...
            $playeroff = isset($_GET['playeroff']) ? (int)$_GET['playeroff'] : 0;  // PLAYER OFF SWITCH

            //** START ENTRY ITEMS
            for ($i = 0; $i < $numrows; ++$i) {
                $lid    = @(int)$results[$i]['lid'];
                $sql    =
                    'select l.lid, l.cid, l.title, l.url, l.srctype, l.listtype, l.listurl, l.entryinfo, l.listcache, l.cachetime, l.logourl, l.credit1, l.credit2, l.credit3, l.submitter, l.status, l.date, l.published, l.hits, l.rating, l.votes, l.allowcomments, l.comments, l.player, l.entryperm, l.entrydownperm, l.views, t.description from '
                    . $xoopsDB->prefix('webshow_links')
                    . ' l, '
                    . $xoopsDB->prefix('webshow_text')
                    . " t where l.lid=$lid and t.lid=$lid ORDER by $orderby";
                $result = $xoopsDB->query($sql);
                while (list($lid, $cid, $title, $url, $srctype, $listtype, $listurl, $entryinfo, $listcache, $cachetime, $logourl, $credit1, $credit2, $credit3, $submitter, $status, $date, $published,
                    $hits, $rating, $votes, $allowcomments, $comments, $player, $ws_entryperm, $ws_entrydownperm, $views, $description) = $xoopsDB->fetchRow($result)) {
                    ...
                    if (count(array_intersect($group, $ws_entryperm)) > 0) {
                        ...
                        if ($xoopsUser && $xoopsUser->isAdmin($xoopsModule->mid())) {
                            ...
                            $adminlink = "<a href=\""
                                         ...
                                         . '/admin/index.php?op=modLink&amp;lid='
                                         . $lid
                                         . "\"><img src=\""
                                         . XOOPS_URL
                                         . '/modules/'
                                         . $xoopsModule->getVar('dirname')
                                         . "/assets/images/editicon.gif\" border=\"0\" alt=\""
                                         . _WS_ADMIN
                                         . "\" /></a>";
                        } else {
                            $isadmin   = false;
                        ...
                        if ($xoopsUser) {
                            ...
                            if ($user == $submitter | $isadmin) {
                                $modlink = "<a href=\""
                                           ...
                                           . '/modules/'
                                           . $xoopsModule->getVar('dirname')
                                           . "/assets/images/modify.gif\" border=\"0\" alt=\""
                                           . _WS_MODIFY
                                           . "\" /></a>";
                            }
                        }
                        ...
                        if (!$showplayer) {
                            ...
                            $title = "<a href=\""
                                     ...
                                     . '/modules/'
                                     . $xoopsModule->getVar('dirname')
                                     . "/index.php?lid=$lid&amp;cid=$cid&amp;uid=$submitter&amp;min=$rowid&amp;orderby=$orderby&amp;show=$show\" target=\"_self\" title=\""
                                     . _WS_VIEWMEDIA
                                     . ' - '
                                     . $ltitle
                                     . "\" rel=\"nofollow\">"
                        ...
                        if (in_array('pop', $showinfo) || in_array('pop', $entryinfo)) {
                            $popbutton = "<span onClick= \"window.open('popmovie.php?lid="
                                         . $lid
                                         . "','popmovie"
                                         . $lid
                                         . "','directories=no,toolbar=no,location=no,menubar=no');return false;\" style=\"cursor:pointer;\"><img src=\""
                                         . XOOPS_URL
                                         . '/modules/'
                                         . $xoopsModule->getVar('dirname')
                                         . "/assets/images/popbutton.gif\" alt=\""
                                         . $ltitle
                                         . ' '
                                         . _WS_POPUP
                                         . ' '
                        ...
                        if ($showplayer & !$playeroff & $lid == $movielid) {
                            ...
                            include __DIR__ . '/include/movie.inc.php';
                            ...
                            $playerresult = $xoopsDB->query('select bgcolor, backcolor, frontcolor, lightcolor, width, height, styleoption from '
                            ...
                            include __DIR__ . '/include/codebox.inc.php';
                            ...
                            $xoopsTpl->assign('link', array(
                                ...
                                'feedlink'      => $feedlink,
                                'feedcode'      => $feedcode,
                                'mail_subject'  => rawurlencode(sprintf(_WS_INTRESTLINK, $xoopsConfig['sitename'])),
                                'mail_body'     => rawurlencode(sprintf(_WS_INTLINKFOUND, $xoopsConfig['sitename'])
                                                                . ':  '
                                                                . XOOPS_URL
                                                                . '/modules/'
                                                                . $xoopsModule->getVar('dirname')
